...
> 🔑 **협업자는 머지(운영 반영) 권한도 가집니다.** 저장소 주인의 승인을 기다릴 필요 없이
> 본인이 올린 PR을 스스로 머지해도 됩니다. 다만 아래 규칙은 지켜주세요.

| 규칙 | 이유 |
|---|---|
| **`main`에 직접 푸시하지 말고 PR을 거칠 것** (머지는 본인이 해도 됨) | 변경 이력이 PR 단위로 남아 **문제 시 클릭 한 번으로 되돌릴 수 있습니다** |
...
> 검증 환경이 마땅치 않으면 **PR을 올리고 설명에 "검증 부탁드립니다"라고 적어주세요.** 저장소 주인이 헤드리스 브라우저 + 실데이터로 검증해 드립니다.

## 5. 머지(운영 반영)

**본인이 직접 머지해도 됩니다.** PR 화면의 **Merge pull request** 버튼을 누르면 1~2분 뒤 라이브에 반영됩니다.

- ✅ **바로 머지해도 좋은 경우**: 문구·색상·설명 수정, 명백한 버그 픽스, 셀프 체크를 통과한 변경
- 🤝 **검토를 받는 편이 좋은 경우**: 신호 판정 로직/임계값 변경, 여러 보드에 걸친 수정, 데이터 소스 추가
  → PR 설명에 "검토 부탁드립니다"라고 적고 저장소 주인에게 알려주세요

### 머지 후 확인
1~2분 뒤 실제 사이트에서 눈으로 확인해 주세요 (캐시가 남으면 `Ctrl+Shift+R`):
- https://topquant-dashboards.vercel.app/ (더 빨리 반영)
- https://topbanker99-ai.github.io/topquant-dashboards/

### 문제가 생겼다면 — 되돌리기
당황하지 마세요. 머지한 PR 화면에서 **`Revert`** 버튼 → 생성된 PR을 머지하면 이전 상태로 복구됩니다.

- 수정 요청이 달리면 **같은 브랜치에 추가 커밋**하면 PR에 자동 반영됩니다(새 PR 불필요).
